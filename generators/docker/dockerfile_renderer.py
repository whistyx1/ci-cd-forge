...
def _generate_multistage_dockerfile(config: DockerfileConfig) -> str:
    ...
    lines.append(f'COPY --from=builder {artifact_source} {artifact_destination}')
...
def generate_dockerfile(config: DockerfileConfig) -> str:
    ...
    if strategy == 'multi':
        return _generate_multistage_dockerfile(config)
